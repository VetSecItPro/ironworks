 ...
 * to cost_events for today's data (rollup always lags by one day).
 *
 * Auth: board role required (assertBoard + assertCompanyAccess). Viewers are
 * blocked at assertCompanyAccess — the companyIds check catches cross-company
 * access while assertBoard blocks non-authenticated actors.
 *
 * Routes:
...
import { type CostRange, costAnalyticsService, type GroupBy, resolveDateRange } from "../services/cost-analytics.js";
...
function parseRange(raw: unknown): CostRange {
  ...
  if (!VALID_RANGES.has(s)) {
    throw badRequest(`Invalid range "${s}". Must be one of: 7d, 30d, 90d, mtd, ytd`);
  }
...
function parseGroupBy(raw: unknown): GroupBy {
  ...
  if (!VALID_GROUP_BY.has(s)) {
    throw badRequest(`Invalid group_by "${s}". Must be one of: day, agent, adapter`);
  }
  return s as GroupBy;
}
  ...
  router.get("/companies/:companyId/costs/time-series", async (req, res) => {
    ...
    res.json({
      ...
      points: points.map((p) => ({
        ...
        // Convert micro-USD to USD for API consumers
