/**
 * Server-side SVG renderer for Ironworks org charts.
...
function subtreeWidth(node: OrgNode): number {
  ...
  const childrenW = node.reports.reduce((sum, child, i) => sum + subtreeWidth(child) + (i > 0 ? GAP_X : 0), 0);
...
function defaultRenderCard(ln: LayoutNode, theme: StyleTheme): string {
  ...
  const shadowFilter = theme.cardShadow ? `filter="url(#${filterId})"` : "";
...
// Ironworks logo: scaled icon (~16px) + wordmark (13px), vertically centered
const IRONWORKS_LOGO_SVG = `<g>
  ...
  <text x="22" y="11.5" font-family="system-ui, -apple-system, sans-serif" font-size="13" font-weight="600" fill="currentColor">Ironworks</text>
</g>`;
...
function smartCollapseTree(roots: OrgNode[]): OrgNode[] {
  ...
  const clone = (nodes: OrgNode[]): OrgNode[] => nodes.map((n) => ({ ...n, reports: clone(n.reports ?? []) }));
...
export async function renderOrgChartPng(
  ...
  return sharp(Buffer.from(svg), { density: 144 }).resize(TARGET_W, TARGET_H).png().toBuffer();
}
