...
if (existsSync(IRONWORKS_ENV_FILE_PATH)) {
  loadDotenv({ path: IRONWORKS_ENV_FILE_PATH, override: false, quiet: true });
}
...
export function loadConfig(): Config {
  ...
  const storageProviderFromEnvRaw = process.env.IRONWORKS_STORAGE_PROVIDER;
  ...
  const storageS3Endpoint = process.env.IRONWORKS_STORAGE_S3_ENDPOINT ?? fileStorage?.s3?.endpoint ?? undefined;
  const storageS3Prefix = process.env.IRONWORKS_STORAGE_S3_PREFIX ?? fileStorage?.s3?.prefix ?? "";
  const storageS3ForcePathStyle =
    process.env.IRONWORKS_STORAGE_S3_FORCE_PATH_STYLE !== undefined
      ? process.env.IRONWORKS_STORAGE_S3_FORCE_PATH_STYLE === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);

  const deploymentModeFromEnvRaw = process.env.IRONWORKS_DEPLOYMENT_MODE;
