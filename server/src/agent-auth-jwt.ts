...
function jwtConfig() {
  ...
  return {
    secret,
    ttlSeconds: parseNumber(process.env.IRONWORKS_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.IRONWORKS_AGENT_JWT_ISSUER ?? "ironworks",
    audience: process.env.IRONWORKS_AGENT_JWT_AUDIENCE ?? "ironworks-api",
  };
}
