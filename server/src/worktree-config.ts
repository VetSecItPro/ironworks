...
import type { IronworksConfig } from "@ironworksai/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Ironworks worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.IRONWORKS_IN_WORKTREE !== "true") return null;

  const configPath = resolveIronworksConfigPath(overrideConfigPath);
  const envPath = resolveIronworksEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.IRONWORKS_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.IRONWORKS_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.IRONWORKS_HOME) ?? nonEmpty(env.IRONWORKS_WORKTREES_DIR) ?? "~/.ironworks-worktrees",
  );
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".ironworks", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as IronworksConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: IronworksConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
function needsWorktreeConfigRepair(config: IronworksConfig, context: WorktreeRuntimeContext): boolean {
...
): { config: IronworksConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.IRONWORKS_HOME = context.homeDir;
  process.env.IRONWORKS_INSTANCE_ID = context.instanceId;
  process.env.IRONWORKS_CONFIG = context.configPath;
  process.env.IRONWORKS_CONTEXT = context.contextPath;
  process.env.IRONWORKS_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as IronworksConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    IRONWORKS_HOME: context.homeDir,
    IRONWORKS_INSTANCE_ID: context.instanceId,
    IRONWORKS_CONFIG: context.configPath,
    IRONWORKS_CONTEXT: context.contextPath,
    IRONWORKS_IN_WORKTREE: "true",
    IRONWORKS_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: { serverPort: number; databasePort?: number | null }): void {
  ...
  let fileConfig: IronworksConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as IronworksConfig;
  } catch {
    return;
  }
