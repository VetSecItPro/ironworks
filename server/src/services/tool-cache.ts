/**
 * In-process LRU cache for first-party IronWorks read-function results.
 *
 * Design decisions:
 *
 * BACKEND — in-memory LRU per-worker, not Redis.
 * Tool call volume is low (typically <100 distinct arg combinations per
 * agent run), and the latency benefit is only meaningful when the DB
 * round-trip is avoided entirely. An in-memory LRU achieves that at zero
 * infrastructure cost. Redis would add a network hop that partially negates
 * the savings and requires operational overhead not yet warranted. If a
 * future Redis instance becomes available (e.g. for shared rate-limit state
 * already planned), migration is straightforward: replace `ToolCache` with a
 ...
 *
 * CACHE KEY — SHA-256 of `{ companyId, toolName, argsSubset }`.
 * `companyId` is the privacy boundary: every cached entry is scoped to the
 * company that produced it, so Company A's "team directory" result can never
 * be served to Company B even when args + tool name match. This is the
 * primary tenant-isolation guarantee for the cache layer; tests assert it
 * explicitly. SHA-256 is overkill for security here (there's no adversarial
 * key-forging risk); it's used because Node's built-in `crypto.createHash`
 * is available without extra deps and produces a compact, fixed-length key
 * (64 hex chars) with negligible collision probability.
 *
 * ERROR RESULTS — never cached.
 * A tool error is often transient (network blip, rate limit, transient DB
 * failure). Caching it would serve the stale error to subsequent callers
 * for the full TTL window, masking recovery. Successful results are safe to
 * cache because idempotent read-only tools return the same data for the
 * same args (within the TTL window).
 *
 * INVALIDATION — TTL expiry only (v1).
 * Manual invalidation adds API surface (and therefore attack surface and
 * operational complexity). TTL expiry is sufficient for the declared use
 * cases. A manual `invalidate(key)` method can be added when a concrete
 * need is identified.
 */
 ...
 *
 * `T` is the cached value type. The cache implementation is generic so
 * different read functions can cache their own result shapes.
 */
...
/**
 * Cache config for first-party IronWorks read functions.
 */
...
/**
 * Build a deterministic cache key for a first-party IronWorks read function.
 *
 ...
 *
 * Tenant isolation via `companyId`: Company A's team directory must never be
 * served to Company B's agent. The cache key embeds `companyId` so two
 * companies with otherwise-identical args produce different keys.
 */
