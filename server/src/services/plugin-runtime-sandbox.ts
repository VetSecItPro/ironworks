...
export async function loadPluginModuleInSandbox(options: PluginSandboxOptions): Promise<LoadedModule> {
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __ironworks_exports: module.exports,
      __ironworks_module: module,
      __ironworks_require: requireInSandbox,
      __ironworks_filename: realPath,
      __ironworks_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__ironworks_exports, __ironworks_module, __ironworks_require, __ironworks_filename, __ironworks_dirname)`;
