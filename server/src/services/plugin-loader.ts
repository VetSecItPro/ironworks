 ...
 *    (`~/.ironworks/plugins/`) and `node_modules` for packages matching
 *    the `ironworks-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Ironworks plugins.
 * Packages matching this pattern are considered Ironworks plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "ironworks-plugin-";
 ...
 */
export const DEFAULT_LOCAL_PLUGIN_DIR = path.join(os.homedir(), ".ironworks", "plugins");
...
export interface DiscoveredPlugin {
  ...
  manifest: IronworksPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem" // ~/.ironworks/plugins/ local directory
  | "npm" // npm packages matching ironworks-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: IronworksPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the ironworks-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "ironworks-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: IronworksPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Ironworks plugins installed as npm packages in the current
   * Node.js environment matching the "ironworks-plugin-*" naming convention.
   *
   ...
   * the "ironworksPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Ironworks plugin.
   * Throws if the package is a Ironworks plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<IronworksPluginManifestV1 | null>;
  ...
  upgradePlugin(
    ...
    oldManifest: IronworksPluginManifestV1;
    newManifest: IronworksPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Ironworks plugin naming convention.
 * Accepts both the "ironworks-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @ironworksai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "ironworksPlugin" key in package.json with a "manifest"
...
function getMinimumHostVersion(manifest: IronworksPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumIronworksVersion;
}
 ...
 *   packageName: "ironworks-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: IronworksPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as IronworksPluginManifestV1 | null;
  ...
  async function fetchAndValidate(installOptions: PluginInstallOptions): Promise<DiscoveredPlugin> {
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Ironworks plugin (no manifest found).`,
      );
    }
   ...
   */
  async function loadManifestFromPath(manifestPath: string): Promise<IronworksPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Ironworks plugin.
   */
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<IronworksPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: IronworksPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @ironworksai/shared exports). Run those workers through
