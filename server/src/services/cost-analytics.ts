 ...
 * falls back to cost_events for today's data (rollup hasn't run yet for today).
 *
...
// cost_events.costCents × 10_000 = micro-USD (1 cent = 10_000 micro-USD)
// Integer multiplication keeps arithmetic in bigint space.

async function fetchTodayFromEvents(
  db: Db,
  companyId: string,
  groupBy: GroupBy,
...
export function costAnalyticsService(db: Db) {
  ...
  async function leaderboard(companyId: string, range: CostRange, limit: number): Promise<LeaderboardEntry[]> {
    ...
    const map = new Map<
      string,
      { agentId: string; agentName: string; callCount: number; inputTokens: number; outputTokens: number; costUsdMicro: number }
    >();
  ...
  async function mom(companyId: string): Promise<MomComparison> {
    ...
    const [curRollup, curToday, prevData] = await Promise.all([
      ...
      prevTo <= today ? rollupTotals(prevFrom, prevTo) : eventsTotals(prevFrom, prevTo),
    ]);
