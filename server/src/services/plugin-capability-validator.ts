...
export interface PluginCapabilityValidator {
   ...
   */
  hasCapability(manifest: IronworksPluginManifestV1, capability: PluginCapability): boolean;

  /**
   * Check whether a plugin has all of the specified capabilities.
   */
  hasAllCapabilities(manifest: IronworksPluginManifestV1, capabilities: PluginCapability[]): CapabilityCheckResult;

  /**
   * Check whether a plugin has at least one of the specified capabilities.
   */
  hasAnyCapability(manifest: IronworksPluginManifestV1, capabilities: PluginCapability[]): boolean;
   ...
   */
  checkOperation(manifest: IronworksPluginManifestV1, operation: string): CapabilityCheckResult;

  /**
   * Assert that a plugin is allowed to perform an operation.
   * Throws a 403 HttpError if the capability check fails.
   */
  assertOperation(manifest: IronworksPluginManifestV1, operation: string): void;

  /**
   * Assert that a plugin has a specific capability.
   * Throws a 403 HttpError if the capability is missing.
   */
  assertCapability(manifest: IronworksPluginManifestV1, capability: PluginCapability): void;

  /**
   * Check whether a plugin can register the given UI slot type.
   */
  checkUiSlot(manifest: IronworksPluginManifestV1, slotType: PluginUiSlotType): CapabilityCheckResult;
   ...
   */
  validateManifestCapabilities(manifest: IronworksPluginManifestV1): CapabilityCheckResult;
...
export function pluginCapabilityValidator(): PluginCapabilityValidator {
  ...
  function capabilitySet(manifest: IronworksPluginManifestV1): Set<PluginCapability> {
  ...
  return {
    ...
    validateManifestCapabilities(manifest) {
      ...
      for (const [feature, requiredCap] of Object.entries(FEATURE_CAPABILITIES)) {
        const featureValue = manifest[feature as keyof IronworksPluginManifestV1];
