...
export function heartbeatService(db: Db) {
  ...
  async function getTaskSession(companyId: string, agentId: string, adapterType: string, taskKey: string) {
    return db
      .select()
  ...
  async function getLatestRunForSession(agentId: string, sessionId: string, opts?: { excludeRunId?: string | null }) {
    const conditions = [eq(heartbeatRuns.agentId, agentId), eq(heartbeatRuns.sessionIdAfter, sessionId)];
  ...
  async function resolveSessionBeforeForWakeup(agent: typeof agents.$inferSelect, taskKey: string | null) {
  ...
  async function resolveWorkspaceForRun(
    ...
    const unorderedProjectWorkspaceRows = workspaceProjectId
      ? await db
          ...
          .where(
            and(eq(projectWorkspaces.companyId, agent.companyId), eq(projectWorkspaces.projectId, workspaceProjectId)),
          )
      ...
      for (const workspace of projectWorkspaceRows) {
        ...
        if (projectCwdExists) {
          return {
            ...
            workspaceHints,
            warnings: [preferredWorkspaceWarning, managedWorkspaceWarning].filter((value): value is string =>
              Boolean(value),
            ),
          };
        }
  ...
  async function clearTaskSessions(
    ...
    const conditions = [eq(agentTaskSessions.companyId, companyId), eq(agentTaskSessions.agentId, agentId)];
    if (opts?.taskKey) {
  ...
  async function setRunStatus(runId: string, status: string, patch?: Partial<typeof heartbeatRuns.$inferInsert>) {
  ...
  async function persistRunProcessMetadata(runId: string, meta: { pid: number; startedAt: string }) {
  ...
  async function claimQueuedRun(run: typeof heartbeatRuns.$inferSelect) {
    ...
    const iterationBlock = await checkIterationLimits(run.agentId, run.companyId, readNonEmptyString(context.issueId));
    if (iterationBlock) {
  ...
  async function finalizeAgentStatus(agentId: string, outcome: "succeeded" | "failed" | "cancelled" | "timed_out") {
    ...
    const nextStatus =
      runningCount > 0 ? "running" : outcome === "succeeded" || outcome === "cancelled" ? "idle" : "error";
    ...
    if (outcome === "failed" && nextStatus === "error") {
      ...
      const recentRuns = await db
        ...
        .from(heartbeatRuns)
        .where(and(eq(heartbeatRuns.agentId, agentId), inArray(heartbeatRuns.status, TERMINAL_STATUSES)))
    ...
    if (!shouldAutoPause && runningCount === 0) {
      try {
        ...
        const agentHourlyRuns = await db
          ...
          .from(heartbeatRuns)
          .where(and(eq(heartbeatRuns.agentId, agentId), gt(heartbeatRuns.createdAt, oneHourAgo)))
        ...
        if (!shouldRatePause) {
          const clusterHourlyRuns = await db
            .select({ count: sql<number>`COUNT(*)::int` })
            .from(heartbeatRuns)
            .where(and(eq(heartbeatRuns.companyId, existing.companyId), gt(heartbeatRuns.createdAt, oneHourAgo)))
            .then((rows) => rows[0]?.count ?? 0);
    ...
    if (updated) {
      ...
      if (shouldAutoPause) {
        logger.warn(
          { agentId, companyId: updated.companyId, consecutiveFailureLimit },
          "Agent auto-paused after consecutive failures",
        );
    ...
    try {
      ...
      // If >80% of gate: reduce maxOutputTokens by 50% (throttle).
      {
        ...
        const lifecycleStage =
          typeof agentMeta.lifecycleStage === "string"
        ...
        if (dailyGateCents > 0) {
          ...
          if (dailySpendCents >= dailyGateCents * 0.8) {
            ...
            const originalCap = resolveMaxOutputTokens(
              parseObject(agent.adapterConfig),
              context,
              run.invocationSource ?? null,
            );
      ...
      if ((run.invocationSource === "timer" || readNonEmptyString(context.reason) === "heartbeat_timer") && !issueId) {
      ...
      const projectExecutionWorkspacePolicy = executionProjectId
        ...
        : null;
      const taskSession = taskKey ? await getTaskSession(agent.companyId, agent.id, agent.adapterType, taskKey) : null;
      ...
      const filteredSkillEntries = skillAllowlist
        ? runtimeSkillEntries.filter(
            (entry: { key: string; required?: boolean }) =>
              entry.required ||
              skillAllowlist.some((allowed: string) => entry.key === allowed || entry.key.includes(allowed)),
          )
        : runtimeSkillEntries;
      ...
      if (issueId && persistedExecutionWorkspace) {
        const nextIssueWorkspaceMode = issueExecutionWorkspaceModeForPersistedWorkspace(
      ...
      if (onboardingPacket && typeof onboardingPacket === "object") {
        ...
        if (onboardingPacket.kbPageSummaries && onboardingPacket.kbPageSummaries.length > 0) {
          const kbLines = onboardingPacket.kbPageSummaries.map((p) => `- **${p.title}** (${p.slug}): ${p.excerpt}`);
          sections.push(`## Key Knowledge\n${kbLines.join("\n")}`);
        }
        if (onboardingPacket.teamContacts && onboardingPacket.teamContacts.length > 0) {
          const contactLines = onboardingPacket.teamContacts.map((c) => `- ${c.name} (${c.role})`);
      ...
      await injectTaskTypeClassification(
        db,
        context,
        issueId,
        issueContext ? { title: issueContext.title ?? "" } : null,
      );
      ...
      try {
        ...
        const routingEnabled =
          typeof runtimeConfigAny.modelRoutingEnabled === "boolean" ? runtimeConfigAny.modelRoutingEnabled : true;
        ...
        const taskComplexity = classifyTaskComplexity({
          ...
          issueCount: Array.isArray(context.issueIds)
            ? context.issueIds.length
        ...
        try {
          ...
          /* non-fatal: escalation flag check must not block the run */
        }
        ...
        const councilConfig: CouncilConfig = {
          strategy:
            (runtimeConfigAny.modelStrategy as string as "single" | "cascade" | "council") ??
            roleDefaults?.strategy ??
            "single",
          ...
          logger.info(
            {
              ...
              strategy: "council",
              models: resolvedStrategy.models,
            },
            ...
            for (const marker of COMPLETION_MARKERS) {
              ...
              if (markerIdx !== -1) {
                ...
                if (afterMarker.length > 500) {
                  ...
                  (adapterResult as unknown as Record<string, unknown>).summary = output.slice(
                    0,
