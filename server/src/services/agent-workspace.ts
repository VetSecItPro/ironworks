...
export async function createAgentDocument(
  ...
  opts: {
    ...
    /** If set, marks this page as a deliverable in the given status. */
...
// ── Tech Debt Register ─────────────────────────────────────────────────────────
//
// Maintains a living markdown-table document in the CTO agent's workspace.
// Each entry appends a row: | Title | Severity | Description | Logged |

const TECH_DEBT_REGISTER_SLUG_PREFIX = "tech-debt-register";

/**
 * Create or append to the Tech Debt Register knowledge page for a company.
 * The register is stored under the CTO agent's workspace folder as a markdown table.
 */
export async function updateTechDebtRegister(
  db: Db,
  companyId: string,
  entry: { title: string; severity: string; description: string },
): Promise<void> {
  const { title, severity, description } = entry;
  const loggedDate = new Date().toLocaleDateString("en-CA", { timeZone: "America/Chicago" });

  // Find the CTO agent for this company (primary CTO, or first engineer-level agent)
  ...
  const ownerAgentId = ctoAgent?.id ?? null;

  // Find existing register page
  const slug = `${TECH_DEBT_REGISTER_SLUG_PREFIX}-${companyId.slice(0, 8)}`;
  const [existing] = await db
    .select({ id: knowledgePages.id, body: knowledgePages.body })
    .from(knowledgePages)
    .where(and(eq(knowledgePages.companyId, companyId), eq(knowledgePages.slug, slug)))
    .limit(1);

  const newRow = `| ${title.replace(/\|/g, "-")} | ${severity} | ${description.replace(/\|/g, "-")} | ${loggedDate} |`;
  ...
  if (existing) {
    ...
    await db
      ...
      .where(eq(knowledgePages.id, existing.id));
    ...
    await db.insert(knowledgePageRevisions).values({
      ...
      body: updatedBody,
      changeSummary: `Added tech debt entry: ${title}`,
      editedByAgentId: ownerAgentId,
      editedByUserId: null,
    });
  } else {
    // Create fresh register with header
    const header = [
      "# Tech Debt Register",
      "",
      "A living inventory of known technical debt, updated automatically by engineering agents.",
      "",
      "| Title | Severity | Description | Logged |",
      "|-------|----------|-------------|--------|",
      newRow,
    ].join("\n");
    ...
    if (page) {
      await db.insert(knowledgePageRevisions).values({
        ...
        title: "Tech Debt Register",
        body: header,
        changeSummary: "Created tech debt register",
        editedByAgentId: ownerAgentId,
        editedByUserId: null,
      });
    }
  ...
  logger.info({ companyId, title, severity }, "tech debt register updated");
}
