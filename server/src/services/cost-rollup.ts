 ...
 *
 * Aggregates the previous day's cost_events into cost_rollup_daily so that
 ...
 * inserting fresh aggregates. Re-running for the same day is safe.
 *
 ...
 * (yesterday UTC) inclusive, backfilling any gaps from server downtime.
 *
 * Empty days: if cost_events has no rows for a target day, the job skips
 * inserting rows for that day and still advances the watermark.
 */
...
export async function runCostRollup(db: Db): Promise<RollupJobResult> {
  ...
  const firstDay = watermark
    ? addDays(watermark, 1)
    : addDays(yesterday, -(DEFAULT_BACKFILL_DAYS - 1));
  ...
  for (const day of days) {
    try {
      ...
      return { daysProcessed: days.indexOf(day), totalRowsInserted, newWatermark: addDays(day, -1) };
    }
  }
