 ...
 *
 * The rollup service is tested by injecting a minimal DB stub rather than
 * using vi.mock() singleton patterns (which the chore/t1-server-test-fixture-refactor
 * branch is deprecating). Each test builds its own stub with controlled behaviour.
 *
 * Covers:
 ...
 * module-level vi.mock() to avoid the singleton caching issues.
 */
function buildStubDb({ watermarkDate = null, aggRowCount = 0 }: StubOptions = {}) {
  ...
  function makeChain(resolveWith: unknown) {
    ...
    for (const m of [
      "from", "where", "leftJoin", "innerJoin", "groupBy", "orderBy",
      "limit", "set", "values", "onConflictDoUpdate", "returning",
    ]) {
  ...
  const db = {
    ...
    select: vi.fn().mockImplementation(() => {
      ...
      if (idx === 0) {
        // First select = watermark read
        return makeChain(
          watermarkDate !== null
            ? [{ general: { costRollupWatermark: watermarkDate }, singletonKey: "default" }]
            : [],
        );
      }
      // Subsequent selects = cost_events aggregate
...
describe("runCostRollup", () => {
  ...
  describe("when watermark is null (first run ever)", () => {
    it("processes 90 days and advances watermark to yesterday", async () => {
      ...
      // 90-day initial backfill with empty days
  ...
  describe("idempotency", () => {
    it("returns no-op result when watermark is already at yesterday", async () => {
      const yesterday = daysBefore(1);
      const db = buildStubDb({ watermarkDate: yesterday });
