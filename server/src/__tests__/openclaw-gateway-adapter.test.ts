...
import { execute, testEnvironment } from "@ironworksai/adapter-openclaw-gateway/server";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
        ...
        buildContext(
          ...
          },
          {
            ...
            context: {
              ...
              ironworksRuntimeServiceIntents: [
                {
                  name: "preview",
                  lifecycle: "ephemeral",
                },
      ...
      expect(payload?.sessionKey).toBe("ironworks:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("IRONWORKS_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("IRONWORKS_TASK_ID=task-123");
