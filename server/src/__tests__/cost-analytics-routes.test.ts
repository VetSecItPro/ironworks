 ...
 * today-fallback, empty-day handling, cross-company 403.
 */
...
// ── Service mock ─────────────────────────────────────────────────────────────
// Mock the service so route tests exercise route logic in isolation
// (param parsing, auth, USD conversion) without a real DB.
...
describe("GET /api/companies/:companyId/costs/time-series", () => {
  ...
  it("returns 403 when actor is unauthenticated (assertBoard rejects non-board actors)", async () => {
    const res = await request(makeApp(noActor())).get(`/api/companies/${COMPANY_ID}/costs/time-series?range=7d`);
    expect(res.status).toBe(403);
  });

  it("returns 403 when company does not belong to the actor", async () => {
    const res = await request(makeApp()).get(`/api/companies/${randomUUID()}/costs/time-series?range=7d`);
    expect(res.status).toBe(403);
  });

  it("returns 400 for an invalid range value", async () => {
    const res = await request(makeApp()).get(`/api/companies/${COMPANY_ID}/costs/time-series?range=365d`);
  ...
  it("returns 200 with micro-USD correctly converted to USD", async () => {
    const res = await request(makeApp()).get(`/api/companies/${COMPANY_ID}/costs/time-series?range=7d&group_by=agent`);
  ...
  it("defaults to group_by=day when group_by is omitted", async () => {
    mockTimeSeries.mockResolvedValue([]);
    const res = await request(makeApp()).get(`/api/companies/${COMPANY_ID}/costs/time-series?range=30d`);
  ...
  it("accepts all valid range values", async () => {
    ...
    for (const range of ["7d", "30d", "90d", "mtd", "ytd"]) {
      const res = await request(app).get(`/api/companies/${COMPANY_ID}/costs/time-series?range=${range}`);
      expect(res.status).toBe(200);
    }
  });

  it("returns empty points array for a day with no data (empty-day handling)", async () => {
    mockTimeSeries.mockResolvedValue([]);
    const res = await request(makeApp()).get(`/api/companies/${COMPANY_ID}/costs/time-series?range=7d`);
...
describe("GET /api/companies/:companyId/costs/leaderboard", () => {
  ...
  it("returns 403 when actor is unauthenticated (assertBoard rejects non-board actors)", async () => {
    const res = await request(makeApp(noActor())).get(`/api/companies/${COMPANY_ID}/costs/leaderboard`);
    expect(res.status).toBe(403);
  });

  it("returns 400 for limit > 100", async () => {
    const res = await request(makeApp()).get(`/api/companies/${COMPANY_ID}/costs/leaderboard?limit=101`);
    expect(res.status).toBe(400);
  });

  it("returns 400 for limit = 0", async () => {
    const res = await request(makeApp()).get(`/api/companies/${COMPANY_ID}/costs/leaderboard?limit=0`);
    expect(res.status).toBe(400);
  });

  it("returns 200 with entries and correct USD conversion", async () => {
    const res = await request(makeApp()).get(`/api/companies/${COMPANY_ID}/costs/leaderboard?range=30d&limit=5`);
  ...
  it("defaults limit to 10 when omitted", async () => {
    const res = await request(makeApp()).get(`/api/companies/${COMPANY_ID}/costs/leaderboard?range=7d`);
    expect(res.status).toBe(200);
    expect(mockLeaderboard).toHaveBeenCalledWith(COMPANY_ID, "7d", 10);
  });

  it("returns empty entries when no agents have spend (today-fallback empty)", async () => {
    mockLeaderboard.mockResolvedValue([]);
    const res = await request(makeApp()).get(`/api/companies/${COMPANY_ID}/costs/leaderboard?range=7d`);
...
describe("GET /api/companies/:companyId/costs/mom", () => {
  ...
  it("returns 200 with MoM data converted to USD", async () => {
  ...
  it("returns null deltaPct when previous month had no spend (today-fallback from zero)", async () => {
    mockMom.mockResolvedValue({
