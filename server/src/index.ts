...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.IRONWORKS_SECRETS_STRICT_MODE === undefined) {
    process.env.IRONWORKS_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.IRONWORKS_EMBEDDED_POSTGRES_VERBOSE === "true";
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.IRONWORKS_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.IRONWORKS_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Ironworks server failed to start");
    process.exit(1);
  });
}
