...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("ironworks-worktree-enabled") !== "true") return null;

  const name = readMetaContent("ironworks-worktree-name");
  const color = normalizeHexColor(readMetaContent("ironworks-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("ironworks-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
