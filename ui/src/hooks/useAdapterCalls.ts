...
export function useAdapterCallList(
  companyId: string | null | undefined,
  query: AdapterCallListQuery = {},
) {
  return useQuery({
    ...
    queryFn: () => {
      ...
      return api.get<AdapterCallListResponse>(
        `/companies/${companyId}/adapter-calls${qs ? `?${qs}` : ""}`,
      );
    },
    staleTime: 30_000,
    retry: false,
...
export function useAdapterCallDetail(
  companyId: string | null | undefined,
  callId: string | null | undefined,
) {
  return useQuery({
    queryKey: ["adapter-calls", companyId, callId, "detail"],
    // Only fetch when both IDs are available — avoids 400 on partial render
