    ...
    <Field label={label}>
      <div className="relative">
        <button
          ...
          className="absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground/70 hover:text-muted-foreground transition-colors"
        >
...
export function OpenClawGatewayConfigFields({ isCreate, values, set, config, eff, mark }: AdapterConfigFieldsProps) {
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Ironworks API URL override">
            <DraftInput
              value={eff("adapterConfig", "ironworksApiUrl", String(config.ironworksApiUrl ?? ""))}
              onCommit={(v) => mark("adapterConfig", "ironworksApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://ironworks.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "ironworks"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Ironworks persists a device key during onboarding so pairing approvals
