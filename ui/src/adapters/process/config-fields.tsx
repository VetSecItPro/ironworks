...
export function ProcessConfigFields({ isCreate, values, set, config, eff, mark }: AdapterConfigFieldsProps) {
  return (
    <>
      ...
      <Field label="Args (comma-separated)" hint={help.args}>
        <DraftInput
          value={isCreate ? values!.args : eff("adapterConfig", "args", formatArgList(config.args))}
          onCommit={(v) =>
            isCreate ? set!({ args: v }) : mark("adapterConfig", "args", v ? parseCommaArgs(v) : undefined)
          }
          immediate
          className={inputClass}
