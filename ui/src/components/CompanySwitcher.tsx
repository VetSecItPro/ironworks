  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="w-full justify-between px-2 py-1.5 h-auto text-left">
          <div className="flex items-center gap-2 min-w-0">
            {selectedCompany && (
              <span className={`h-2 w-2 rounded-full shrink-0 ${statusDotColor(selectedCompany.status)}`} />
            )}
            <span className="text-sm font-medium truncate">{selectedCompany?.name ?? "Select company"}</span>
          </div>
      ...
      <DropdownMenuContent align="start" className="w-[220px]">
        ...
        {sidebarCompanies.length === 0 && <DropdownMenuItem disabled>No companies</DropdownMenuItem>}
        <DropdownMenuSeparator />
