...
import type { AgentPerfRow, SortField, TimeRange } from "../../types/performance";

export type { AgentPerfRow, SortField, TimeRange };
...
export function computeAgentPerformance(
  ...
  if (rows.length > 0) {
    ...
    const avgCost =
      withTasks.length > 0 ? withTasks.reduce((s, r) => s + (r.costPerTask ?? 0), 0) / withTasks.length : 0;
    const avgClose =
      withTasks.filter((r) => r.avgCloseH !== null).reduce((s, r) => s + r.avgCloseH!, 0) /
    ...
    for (const row of rows) {
      ...
      const composite = Math.round(
        costScore * 0.25 + speedScore * 0.25 + throughputScore * 0.25 + completionScore * 0.25,
      );
