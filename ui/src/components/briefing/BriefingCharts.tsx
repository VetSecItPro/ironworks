...
export function WeeklyLineChart({
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${CHART_W} ${CHART_H + 20}`} className="w-full" aria-label="Weekly spend trend chart">
        <line
          x1={CHART_PAD_X}
        ...
        {points.map((p, i) => (
          <g key={p.d.label}>
            ...
            <text x={p.x} y={CHART_H + 18} textAnchor="middle" fontSize={8} fill="currentColor" opacity={0.5}>
              {p.d.label}
            </text>
          </g>
        ))}
...
export function WeeklyBarChart({ data }: { data: Array<{ label: string; value: number }> }) {
  const max = Math.max(...data.map((d) => d.value), 1);
  const n = data.length;
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={d.label}>
              ...
              />
              {d.value > 0 && (
                <text x={x + barW / 2} y={y - 3} textAnchor="middle" fontSize={9} fill="currentColor" opacity={0.8}>
                  {d.value}
                </text>
              )}
              <text
