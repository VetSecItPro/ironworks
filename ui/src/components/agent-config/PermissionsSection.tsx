...
import { DraftInput, DraftNumberInput, Field, help } from "../agent-config-primitives";
import { MarkdownEditor } from "../MarkdownEditor";
import { EnvVarEditor } from "./EnvVarEditor";
import { ModelDropdown } from "./ModelDropdown";
import { ThinkingEffortDropdown } from "./ThinkingEffortDropdown";
import type { AdapterModel, Agent, CompanySecret, CreateConfigValues, EnvBinding, SectionCommonProps } from "./types";
import {
      ...
      <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
        ...
        {!isCreate && (
          <Field
            ...
            hint="How tasks are executed: single model, cascade (retry with fallback), or council (multi-model deliberation). Critical tasks auto-upgrade to council."
          >
            <select
        ...
        {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
          <>
            <Field label="Bootstrap prompt (legacy)" hint={help.bootstrapPrompt}>
              <MarkdownEditor
                value={eff("adapterConfig", "bootstrapPromptTemplate", String(config.bootstrapPromptTemplate ?? ""))}
            ...
            <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
              Bootstrap prompt is legacy and will be removed in a future release. Consider moving this content into the
              agent&apos;s prompt template or instructions file instead.
            </div>
          </>
        )}
        ...
        <Field label="Extra args (comma-separated)" hint={help.extraArgs}>
          <DraftInput
            ...
            onCommit={(v) =>
              isCreate ? set!({ extraArgs: v }) : mark("adapterConfig", "extraArgs", v ? parseCommaArgs(v) : undefined)
            }
            immediate
            className={inputClass}
