...
export function InteractiveChart({
  ...
  const getX = useCallback((i: number) => PADDING.left + i * (barWidth + gap) + barWidth / 2, [barWidth, gap, PADDING.left]);
  const getY = useCallback((v: number) => PADDING.top + chartHeight - (v / maxValue) * chartHeight, [chartHeight, maxValue, PADDING.top]);
  ...
  const areaPath = useMemo(() => {
    ...
    return `M ${getX(0)} ${base} ` + visibleData.map((d, i) => `L ${getX(i)} ${getY(d.value)}`).join(" ") + ` L ${getX(visibleData.length - 1)} ${base} Z`;
    ...
    <div ref={containerRef} className={cn("space-y-2", className)}>
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          {comparisonData && (
            <button onClick={() => setShowComparison(!showComparison)} className={cn(
              "flex items-center gap-1 px-2 py-1 text-[10px] font-medium rounded border transition-colors",
              showComparison ? "border-purple-400 bg-purple-500/10 text-purple-400" : "border-border text-muted-foreground hover:bg-accent/50",
            )}>
          ...
          {zoomRange && (
            <Button size="sm" variant="ghost" className="h-6 text-[10px] px-2" onClick={() => setZoomRange(null)}>
              <Maximize2 className="h-3 w-3 mr-0.5" />Reset
            </Button>
          )}
          <Button size="sm" variant="ghost" className="h-6 w-6 p-0" title="Export as PNG"
            onClick={() => { if (svgRef.current) exportChartAsPng(svgRef.current, "chart"); }}>
      ...
      <svg ref={svgRef} width={containerWidth} height={height} className="select-none" onWheel={handleWheel}>
        {yTicks.map((tick, i) => (
          <g key={i}>
            <line x1={PADDING.left} y1={getY(tick)} x2={containerWidth - PADDING.right} y2={getY(tick)} stroke="var(--border)" strokeWidth={0.5} strokeDasharray={i === 0 ? undefined : "3 3"} />
            <text x={PADDING.left - 8} y={getY(tick) + 3} textAnchor="end" fontSize={9} fill="var(--muted-foreground)">{formatValue(tick)}</text>
          </g>
        ))}
        ...
        {annotations.map((ann, i) => {
          ...
          return (
            <g key={`ann-${i}`}>
              <line x1={x} y1={PADDING.top} x2={x} y2={PADDING.top + chartHeight} stroke={ann.color ?? "var(--destructive)"} strokeWidth={1.5} strokeDasharray={ann.dashed ? "4 4" : undefined} opacity={0.7} />
              <text x={x} y={PADDING.top - 4} textAnchor="middle" fontSize={8} fill={ann.color ?? "var(--destructive)"} fontWeight={600}>{ann.label}</text>
            </g>
          );
        })}
        ...
        {showComparison && comparisonLinePath && (
          <path d={comparisonLinePath} fill="none" stroke="var(--chart-2, #a855f7)" strokeWidth={1.5} strokeDasharray="4 4" opacity={0.7} />
        )}

        {chartType === "bar"
          ? visibleData.map((d, i) => (
              <g key={i}>
                <rect x={getX(i) - barWidth / 2} y={getY(d.value)} width={barWidth} height={Math.max(0, PADDING.top + chartHeight - getY(d.value))} rx={2}
                  fill={hoveredIndex === i ? "var(--primary)" : "var(--primary)"} opacity={hoveredIndex === i ? 1 : 0.7}
                  className="transition-opacity cursor-pointer"
                  onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)}
                  onClick={() => onPointClick?.(d, zoomRange ? i + zoomRange[0] : i)} />
                {showComparison && visibleComparison?.[i] && (
                  <rect x={getX(i) - barWidth / 2 + barWidth * 0.4} y={getY(visibleComparison[i].value)}
                    width={barWidth * 0.5} height={Math.max(0, PADDING.top + chartHeight - getY(visibleComparison[i].value))}
                    rx={2} fill="var(--chart-2, #a855f7)" opacity={0.5} />
                )}
              </g>
            ))
          : (
              <>
                ...
                {visibleData.map((d, i) => (
                  <circle key={i} cx={getX(i)} cy={getY(d.value)} r={hoveredIndex === i ? 5 : 3}
        ...
        {visibleData.map((d, i) => {
          ...
          return (
            <text key={i} x={getX(i)} y={PADDING.top + chartHeight + 16} textAnchor="middle" fontSize={9} fill="var(--muted-foreground)">{d.label}</text>
          );
        })}

        {hoveredIndex !== null && visibleData[hoveredIndex] && (
          <g>
            <rect x={Math.min(getX(hoveredIndex) - 40, containerWidth - PADDING.right - 80)} y={getY(visibleData[hoveredIndex].value) - 28} width={80} height={22} rx={4}
              fill="var(--popover)" stroke="var(--border)" strokeWidth={1} />
            <text x={Math.min(getX(hoveredIndex), containerWidth - PADDING.right - 40)} y={getY(visibleData[hoveredIndex].value) - 14}
              textAnchor="middle" fontSize={10} fontWeight={600} fill="var(--foreground)">{formatValue(visibleData[hoveredIndex].value)}</text>
          </g>
        )}
      </svg>
    </div>
