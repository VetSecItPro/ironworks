...
import {
  CircleDot, Bot, Hexagon, Target, LayoutDashboard, Inbox, DollarSign,
  History, SquarePen, Plus, BookOpen, FileText, Settings, Keyboard,
  BarChart3, Shield, Users, Bookmark, Search, Trash2,
} from "lucide-react";
    ...
    >
      ...
      <CommandList>
        ...
        <CommandGroup heading="Actions">
          ...
          <CommandItem onSelect={() => { saveCurrentSearch(); setOpen(false); }}>
        ...
        <CommandGroup heading="Pages">
          ...
          <CommandItem onSelect={() => go("/costs")}><DollarSign className="mr-2 h-4 w-4" />Costs<span className="ml-auto text-xs text-muted-foreground">g c</span></CommandItem>
          <CommandItem onSelect={() => go("/activity")}><History className="mr-2 h-4 w-4" />Activity</CommandItem>
        </CommandGroup>

        <CommandSeparator />

        <CommandGroup heading="More Pages">
          <CommandItem onSelect={() => go("/playbooks")}><FileText className="mr-2 h-4 w-4" />Playbooks<span className="ml-auto text-xs text-muted-foreground">g b</span></CommandItem>
          <CommandItem onSelect={() => go("/library")}><BookOpen className="mr-2 h-4 w-4" />Library<span className="ml-auto text-xs text-muted-foreground">g l</span></CommandItem>
          <CommandItem onSelect={() => go("/performance")}><BarChart3 className="mr-2 h-4 w-4" />Performance</CommandItem>
          <CommandItem onSelect={() => go("/audit-log")}><Shield className="mr-2 h-4 w-4" />Audit Log</CommandItem>
          <CommandItem onSelect={() => go("/org")}><Users className="mr-2 h-4 w-4" />Org Chart</CommandItem>
          <CommandItem onSelect={() => go("/company/settings")}><Settings className="mr-2 h-4 w-4" />Company Settings<span className="ml-auto text-xs text-muted-foreground">g s</span></CommandItem>
          <CommandItem onSelect={() => go("/keyboard-shortcuts")}><Keyboard className="mr-2 h-4 w-4" />Keyboard Shortcuts<span className="ml-auto text-xs text-muted-foreground">g k</span></CommandItem>
        </CommandGroup>
