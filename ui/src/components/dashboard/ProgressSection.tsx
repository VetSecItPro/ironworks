...
import { ChartCard, IssueStatusChart, PriorityChart } from "../ActivityCharts";
...
export function ProgressSection({ activeGoals, issues }: { activeGoals: GoalProgress[]; issues: Issue[] }) {
  return (
          ...
          <div className="space-y-4">
            {activeGoals.slice(0, 5).map((goal) => (
              <Link
                ...
                className="block space-y-1.5 no-underline text-inherit hover:opacity-80 transition-opacity"
              >
                ...
                <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                  <span>
                    {goal.completedIssues}/{goal.totalIssues} done
                  </span>
                  {goal.blockedIssues > 0 && <span className="text-amber-400">- {goal.blockedIssues} blocked</span>}
                  {goal.blockedIssues === 0 && goal.progressPercent < 100 && (
                    <span className="text-emerald-400">- on track</span>
                  )}
                  {goal.progressPercent === 100 && <span className="text-emerald-400">- complete</span>}
                </div>
              </Link>
            ))}
          </div>
