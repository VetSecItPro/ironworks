...
function spriteSize(sprite: IronworksSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = IRONWORKS_SPRITES[(baseCol + baseRow) % IRONWORKS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = IRONWORKS_SPRITES[Math.floor(Math.random() * IRONWORKS_SPRITES.length)]!;
  ...
  return (
    <pre
      ref={preRef}
      className="w-full h-full m-0 p-0 overflow-hidden text-muted-foreground/80 select-none leading-none"
