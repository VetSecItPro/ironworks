  ...
  return (
    <div className="border border-border rounded-lg divide-y divide-border">
      {agents.map((agent) => {
        ...
        return (
          <div key={agent.id} className="flex items-center gap-4 px-4 py-3 hover:bg-accent/20 transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                {agent.popular && <Star className="h-3 w-3 text-amber-500 fill-amber-500" />}
            ...
            <div className="flex items-center gap-2 shrink-0">
              <div className="hidden sm:flex gap-1">
                {agent.skills.slice(0, 3).map((s) => (
                  <span key={s} className="text-[10px] px-1.5 py-0.5 rounded-full bg-muted/60 text-muted-foreground">
                    {s}
                  </span>
                ))}
              </div>
