 ...
 *
 * Architecture:
 *   - FilterBar: adapter_type, status, source dropdowns + agent_id text input
 ...
 *   - CompareView: Sheet showing two calls side-by-side
 *
 * Infinite scroll: intersection observer on a sentinel div at the bottom.
 * When the sentinel becomes visible, the cursor is advanced to the next page.
 *
 * Selection: max 2 items; "Compare" button appears when exactly 2 are selected.
 *
 * SSE replay: raw fetch() + ReadableStream; no library needed for a single
 * ndjson/SSE stream. Confirm dialog prevents accidental re-spending.
 *
 * displayItems pattern: falls back to data.items when allItems is empty so
 * the list is visible in SSR/test environments where useEffect does not run.
 */
  ...
  return (
    <div className="flex flex-wrap gap-2 mb-4">
      ...
      <Select
        ...
        onValueChange={(v) =>
          onChange({ ...filters, status: v === "all" ? "" : (v as AdapterCallStatus) })
        }
      >
      ...
      <Select
        ...
        onValueChange={(v) =>
          onChange({ ...filters, source: v === "all" ? "" : (v as AdapterCallSource) })
        }
      >
...
function ColumnHeaders({ onSelectAll }: { onSelectAll?: () => void }) {
  return (
    <div
      ...
      style={{
        gridTemplateColumns: "24px 1fr 120px 80px 80px 80px 80px 90px",
      }}
      aria-label="Column headers"
    >
      {/* biome-ignore lint/a11y/useButtonType: checkbox role */}
      <input type="checkbox" className="h-3 w-3" onChange={onSelectAll} aria-label="Select all" />
...
function CallRow({
  ...
  return (
    // biome-ignore lint/a11y/useKeyWithClickEvents: row click is supplemental
    <div
...
function CallDrawer({
  ...
  function handleReplayConfirm() {
    ...
    // SSE replay via raw fetch + ReadableStream — avoids EventSource (POST-only)
          ...
          for (const frame of frames) {
            ...
            if (dataLine) {
              try {
                ...
                if (payload.chunk) setReplayOutput((prev) => prev + String(payload.chunk));
              } catch {
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will re-send the original prompt to the provider and consume API credits.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
...
export function RequestExplorer({ companyId }: { companyId: string }) {
  ...
  // Reset cursor + accumulated items when filters change
  ...
  // SSR-safe: prefer accumulated allItems, fall back to first page from query
  ...
  const selectedPair =
    selectedIds.size === 2 ? ([...selectedIds] as [string, string]) : null;
      ...
      <div className="rounded-md border border-border overflow-hidden">
        ...
        {error && (
          <div className="px-4 py-3 text-sm text-destructive">
            Failed to load adapter calls: {error.message}
          </div>
        )}
