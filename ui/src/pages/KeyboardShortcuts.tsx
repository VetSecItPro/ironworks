        ...
        {Object.entries(grouped).map(([scope, entries]) => (
          <div key={scope}>
            ...
            <div className="rounded-lg border border-border divide-y divide-border">
              {entries.map((entry) => (
                <div key={entry.description} className="flex items-center justify-between px-4 py-3">
                  <span className="text-sm text-foreground">{entry.description}</span>
                  <div className="flex items-center gap-1">
                    {entry.keys.map((key) =>
                      key === "then" ? (
                        <span key={`then-${entry.description}`} className="text-xs text-muted-foreground mx-0.5">
                          then
                        </span>
                      ) : (
                        <Kbd key={key}>{key}</Kbd>
