 ...
 *   Step 2 — Connect your LLM provider (skipped in CI)
 ...
 * heartbeat fires. Set IRONWORKS_E2E_SKIP_LLM=false to enable LLM-dependent
 * assertions (requires a valid ANTHROPIC_API_KEY).
 */

const SKIP_LLM = process.env.IRONWORKS_E2E_SKIP_LLM !== "false";
...
test.describe("Onboarding wizard", () => {
  test("completes full wizard flow", async ({ page }) => {
    ...
    // Step 2: LLM Provider — skip in CI (no API key available)
    await expect(page.locator("h2", { hasText: "Connect your LLM provider" })).toBeVisible({ timeout: 10_000 });
    await page.getByText("Skip for now").click();

    await expect(page.locator("h3", { hasText: "Build your team" })).toBeVisible({ timeout: 10_000 });
    ...
    await page.getByRole("button", { name: "Create Your AI Company" }).click();
