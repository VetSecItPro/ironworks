...
const ADMIN_EMAIL =
  process.env.IRONWORKS_RELEASE_SMOKE_EMAIL ?? process.env.SMOKE_ADMIN_EMAIL ?? "smoke-admin@ironworks.local";
const ADMIN_PASSWORD =
  process.env.IRONWORKS_RELEASE_SMOKE_PASSWORD ?? process.env.SMOKE_ADMIN_PASSWORD ?? "ironworks-smoke-password";
...
test.describe("Docker authenticated onboarding smoke", () => {
  test("logs in, completes onboarding, and triggers the first CEO run", async ({ page }) => {
    ...
    await page.locator('input[placeholder="e.g. Research competitor pricing"]').fill(TASK_TITLE);
    await page.getByRole("button", { name: "Next" }).click();
