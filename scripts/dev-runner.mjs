...
const devServerStatusFilePath = path.join(repoRoot, ".ironworks", "dev-server-status.json");
...
const env = {
  ...process.env,
  IRONWORKS_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.IRONWORKS_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
}

if (mode === "watch") {
  env.IRONWORKS_MIGRATION_PROMPT ??= "never";
  env.IRONWORKS_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.IRONWORKS_DEPLOYMENT_MODE = "authenticated";
  env.IRONWORKS_DEPLOYMENT_EXPOSURE = "private";
  env.IRONWORKS_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[ironworks] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[ironworks] dev mode: local_trusted (default)");
}
...
async function getMigrationStatusPayload() {
  ...
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[ironworks] Command failed with code ${status.code}: pnpm --filter @ironworksai/db exec tsx src/migration-status.ts --json\n`,
    );
...
async function maybePreflightMigrations(options = {}) {
  ...
  const autoApply = options.autoApply ?? env.IRONWORKS_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[ironworks] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
...
async function buildPluginSdk() {
  ...
  if (result.code !== 0) {
    console.error("[ironworks] plugin sdk build failed");
