/**
 * esbuild configuration for building the ironworksai CLI for npm.
 *
 * Bundles all workspace packages (@ironworksai/*) into a single file.
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@ironworksai/")) {
      externals.add(name);
    }
  }
