...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".ironworks");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: IronworksConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "ironworks",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    IRONWORKS_HOME: paths.homeDir,
    IRONWORKS_INSTANCE_ID: paths.instanceId,
    IRONWORKS_CONFIG: paths.configPath,
    IRONWORKS_CONTEXT: paths.contextPath,
    IRONWORKS_IN_WORKTREE: "true",
    ...(branding?.name ? { IRONWORKS_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { IRONWORKS_WORKTREE_COLOR: branding.color } : {}),
  };
}
