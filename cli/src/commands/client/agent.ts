...
function buildAgentEnvExports(input: { apiBase: string; companyId: string; agentId: string; apiKey: string }): string {
  ...
  return [
    ...
    `export IRONWORKS_COMPANY_ID='${escaped(input.companyId)}'`,
    `export IRONWORKS_AGENT_ID='${escaped(input.agentId)}'`,
    `export IRONWORKS_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
      ...
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveIronworksSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Ironworks skills directory. Expected ./skills in the repo checkout.",
              );
            }
