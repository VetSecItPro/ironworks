...
const DEFAULT_AGENT_JWT_ISSUER = "ironworks";
const DEFAULT_AGENT_JWT_AUDIENCE = "ironworks-api";
...
export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" ironworks env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: IronworksConfig | null = null;
...
function collectDeploymentEnvRows(config: IronworksConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrl =
    ...
    "";
  const publicUrlSource: EnvSource = process.env.IRONWORKS_PUBLIC_URL
    ? "env"
    : process.env.IRONWORKS_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
      ? "env"
      : config?.auth?.publicBaseUrl
        ? "config"
        : "missing";
  ...
  const secretsProvider =
    process.env.IRONWORKS_SECRETS_PROVIDER ?? config?.secrets?.provider ?? DEFAULT_SECRETS_PROVIDER;
  const secretsStrictMode = process.env.IRONWORKS_SECRETS_STRICT_MODE ?? String(config?.secrets?.strictMode ?? false);
  const secretsKeyFilePath =
    process.env.IRONWORKS_SECRETS_MASTER_KEY_FILE ??
    config?.secrets?.localEncrypted?.keyFilePath ??
    defaultSecretsKeyFilePath();
  const storageProvider =
    process.env.IRONWORKS_STORAGE_PROVIDER ?? config?.storage?.provider ?? DEFAULT_STORAGE_PROVIDER;
  const storageLocalDir =
    process.env.IRONWORKS_STORAGE_LOCAL_DIR ?? config?.storage?.localDisk?.baseDir ?? defaultStorageBaseDir();
  const storageS3Bucket = process.env.IRONWORKS_STORAGE_S3_BUCKET ?? config?.storage?.s3?.bucket ?? "ironworks";
  const storageS3Region = process.env.IRONWORKS_STORAGE_S3_REGION ?? config?.storage?.s3?.region ?? "us-east-1";
  const storageS3Endpoint = process.env.IRONWORKS_STORAGE_S3_ENDPOINT ?? config?.storage?.s3?.endpoint ?? "";
  const storageS3Prefix = process.env.IRONWORKS_STORAGE_S3_PREFIX ?? config?.storage?.s3?.prefix ?? "";
  const storageS3ForcePathStyle =
    process.env.IRONWORKS_STORAGE_S3_FORCE_PATH_STYLE ?? String(config?.storage?.s3?.forcePathStyle ?? false);

  const rows: EnvVarRow[] = [
    {
      key: "IRONWORKS_AGENT_JWT_SECRET",
    ...
    },
    {
      ...
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from IRONWORKS_PUBLIC_URL when possible)",
    },
    {
      key: "IRONWORKS_AGENT_JWT_TTL_SECONDS",
      value: process.env.IRONWORKS_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.IRONWORKS_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "IRONWORKS_AGENT_JWT_ISSUER",
      value: process.env.IRONWORKS_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.IRONWORKS_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "IRONWORKS_AGENT_JWT_AUDIENCE",
      value: process.env.IRONWORKS_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.IRONWORKS_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "IRONWORKS_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.IRONWORKS_SECRETS_PROVIDER ? "env" : config?.secrets?.provider ? "config" : "default",
      required: false,
      note: "Default provider for new secrets",
    },
    {
      key: "IRONWORKS_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.IRONWORKS_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "IRONWORKS_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.IRONWORKS_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "IRONWORKS_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.IRONWORKS_STORAGE_PROVIDER ? "env" : config?.storage?.provider ? "config" : "default",
      required: false,
      note: "Storage provider (local_disk or s3)",
    },
    {
      key: "IRONWORKS_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.IRONWORKS_STORAGE_LOCAL_DIR
        ? "env"
    ...
    {
      key: "IRONWORKS_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.IRONWORKS_STORAGE_S3_BUCKET ? "env" : config?.storage?.s3?.bucket ? "config" : "default",
      required: false,
      note: "S3 bucket name for s3 provider",
    },
    {
      key: "IRONWORKS_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.IRONWORKS_STORAGE_S3_REGION ? "env" : config?.storage?.s3?.region ? "config" : "default",
      required: false,
      note: "S3 region for s3 provider",
    },
    {
      key: "IRONWORKS_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.IRONWORKS_STORAGE_S3_ENDPOINT ? "env" : config?.storage?.s3?.endpoint ? "config" : "default",
      required: false,
      note: "Optional custom endpoint for S3-compatible providers",
    },
    {
      key: "IRONWORKS_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.IRONWORKS_STORAGE_S3_PREFIX ? "env" : config?.storage?.s3?.prefix ? "config" : "default",
      required: false,
      note: "Optional object key prefix",
    },
    {
      key: "IRONWORKS_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.IRONWORKS_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.IRONWORKS_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "IRONWORKS_CONFIG",
      value: process.env.IRONWORKS_CONFIG ?? configPath,
      source: process.env.IRONWORKS_CONFIG ? "env" : "default",
      required: false,
