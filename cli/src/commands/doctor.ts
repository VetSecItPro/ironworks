...
export async function doctor(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" ironworks doctor ")));
  ...
  try {
    ...
    const readResult: CheckResult = {
      ...
      canRepair: false,
      repairHint: "Run `ironworksai configure --section database` or `ironworksai onboard`",
    };
...
async function runRepairableCheck(input: {
  ...
  loadIronworksEnvFile(input.configPath);
