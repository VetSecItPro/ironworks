...
const JWT_SECRET_ENV_KEY = "IRONWORKS_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Ironworks CLI commands",
...
export function resolveIronworksEnvFile(configPath?: string): string {
...
export function loadIronworksEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeIronworksEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readIronworksEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeIronworksEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeIronworksEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readIronworksEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeIronworksEnvEntries(next, filePath);
  return next;
}
