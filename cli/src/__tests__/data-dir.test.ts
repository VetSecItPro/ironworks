...
describe("applyDataDirOverride", () => {
  ...
  it("sets IRONWORKS_HOME and isolated default config/context paths", () => {
    ...
    expect(process.env.IRONWORKS_CONFIG).toBe(path.resolve(expectedHome, "instances", "default", "config.json"));
  ...
  it("uses the provided instance id when deriving default config path", () => {
    ...
    expect(process.env.IRONWORKS_CONFIG).toBe(path.resolve("/tmp/ironworks-alt", "instances", "dev_1", "config.json"));
  });
