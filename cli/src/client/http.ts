...
export class IronworksApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-ironworks-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = ["Could not reach the Ironworks API.", "", `Request: ${input.method} ${input.url}`];
  if (input.causeMessage) {
    lines.push(`Cause: ${input.causeMessage}`);
  }
  lines.push(
    "",
    "This usually means the Ironworks server is not running, the configured URL is wrong, or the request is being blocked before it reaches Ironworks.",
    "",
    "Try:",
    "- Start Ironworks with `pnpm dev` or `pnpm ironworksai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Ironworks is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`IRONWORKS_API_URL\`.`,
  );
  return lines.join("\n");
}
