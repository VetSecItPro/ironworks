...
--   agent_id is nullable so playground calls (no agent) can be represented
--   when a future ingestion path adds them.
-- - source: 'agent' | 'playground' | 'unknown'
-- - cost_usd_micro: cost stored as micro-USD (1e-6 USD per unit) for integer
--   arithmetic precision. Divide by 1_000_000 in application code for USD.
-- - watermark: tracked in instance_settings.general.costRollupWatermark so
--   re-runs are idempotent: DELETE + re-INSERT for the target day.
--
-- Authorization is app-layer only (assertCompanyAccess). No RLS — consistent
-- with every other production table in this codebase.
