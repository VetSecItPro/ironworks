...
async function probeEmbeddedPostgresSupport(): Promise<EmbeddedPostgresTestSupport> {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "ironworks-embedded-postgres-probe-"));
...
export async function startEmbeddedPostgresTestDatabase(tempDirPrefix: string): Promise<EmbeddedPostgresTestDatabase> {
  ...
  try {
    ...
    const connectionString = `postgres://ironworks:ironworks@127.0.0.1:${port}/ironworks`;
