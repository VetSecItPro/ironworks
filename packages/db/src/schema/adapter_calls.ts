 ...
 *     Both are intentionally nullable so list queries are cheaper (no large
 *     JSONB columns in the 50-row page fetch).
 ...
 *     so the list view never needs to JOIN to the full payload columns.
...
export const adapterCalls = pgTable(
  ...
  (table) => ({
    // Primary access pattern: list by company ordered by time desc + id desc
    // for stable cursor pagination when multiple calls share a millisecond.
    companyOccurredIdx: index("idx_ac_company_occurred").on(table.companyId, table.occurredAt, table.id),
