...
async function ensureEmbeddedPostgresConnection(dataDir: string, preferredPort: number): Promise<MigrationConnection> {
  ...
  const preferredAdminConnectionString = `postgres://ironworks:ironworks@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "ironworks");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://ironworks:ironworks@127.0.0.1:${preferredPort}/ironworks`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://ironworks:ironworks@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "ironworks");
    return {
      connectionString: `postgres://ironworks:ironworks@127.0.0.1:${port}/ironworks`,
  ...
  const adminConnectionString = `postgres://ironworks:ironworks@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "ironworks");

  return {
    connectionString: `postgres://ironworks:ironworks@127.0.0.1:${selectedPort}/ironworks`,
