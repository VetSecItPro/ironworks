...
function resolveIronworksInstanceId(): string {
  const raw = process.env.IRONWORKS_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveIronworksHomeDir(), "instances", resolveIronworksInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveIronworksHomeDir(), "instances", resolveIronworksInstanceId(), "data", "backups");
}
