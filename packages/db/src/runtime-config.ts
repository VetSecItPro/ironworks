...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "ironworks-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveIronworksHomeDir(), "instances", resolveIronworksInstanceId(), CONFIG_BASENAME);
}

function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveIronworksHomeDir(), "instances", resolveIronworksInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".ironworks", CONFIG_BASENAME);
...
function resolveIronworksConfigPath(): string {
  if (process.env.IRONWORKS_CONFIG?.trim()) {
    return path.resolve(process.env.IRONWORKS_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveIronworksEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveIronworksEnvPath(configPath);
