/**
 * G.8 — Kill-switch test for poe_api adapter.
 *
...
const DISABLE_ENV_KEY = "ADAPTER_DISABLE_POE_API";

describe("poe_api kill-switch (G.8)", () => {
  beforeEach(() => {
  ...
  it("testEnvironment returns fail when kill-switch is active", async () => {
    ...
    const result = await testEnvironment(
      { adapterType: "poe_api", config: { apiKey: "sk-test" } },
      mockTransport as never,
    );
    expect(result.status).toBe("fail");
    expect(result.checks.some((c) => c.code === "poe_api_disabled")).toBe(true);
    expect(mockTransport.sendJson).not.toHaveBeenCalled();
  });

  it("testEnvironment proceeds normally when kill-switch is not set", async () => {
    // No ADAPTER_DISABLE_POE_API set — should attempt network probe (will fail, that is ok)
    const { testEnvironment } = await import("../test.js");
    const mockTransport = {
      sendJson: vi.fn().mockRejectedValue(new Error("network unreachable")),
      sendJsonStream: vi.fn(),
    };
    const result = await testEnvironment({ adapterType: "poe_api", config: {} }, mockTransport as never);
    // Should NOT have the kill-switch check code
    expect(result.checks.every((c) => c.code !== "poe_api_disabled")).toBe(true);
  });
  ...
  it("execute returns disabled error when kill-switch is active", async () => {
    ...
    const ctx = {
      runId: "run-1",
      agent: { id: "a1", companyId: "c1", name: "Test", adapterType: "poe_api", adapterConfig: {} },
      runtime: { sessionId: null, sessionParams: null, sessionDisplayId: null, taskKey: null },
      config: { apiKey: "sk-test", model: "claude-sonnet-4-6" },
      context: { taskId: "t1", issueId: "i1", wakeReason: "test", issueIds: [] },
      onLog: async () => {},
    };
    const result = await execute(ctx as never, mockTransport as never);
    expect(result.exitCode).toBe(1);
    expect(result.errorCode).toBe("poe_api_disabled");
  ...
  it("execute proceeds normally when kill-switch is not set", async () => {
    ...
    const result = await execute(ctx as never, mockTransport as never);
    expect(result.errorCode).not.toBe("poe_api_disabled");
  });
});
