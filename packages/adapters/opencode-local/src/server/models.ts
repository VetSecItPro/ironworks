...
import { asString, ensurePathInEnv, runChildProcess } from "@ironworksai/adapter-utils/server-utils";
...
function resolveOpenCodeCommand(input: unknown): string {
  const envOverride =
    typeof process.env.IRONWORKS_OPENCODE_COMMAND === "string" &&
    process.env.IRONWORKS_OPENCODE_COMMAND.trim().length > 0
      ? process.env.IRONWORKS_OPENCODE_COMMAND.trim()
      : "opencode";
