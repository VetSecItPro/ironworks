...
async function isLikelyIronworksRepoRoot(candidate: string): Promise<boolean> {
...
async function isLikelyIronworksRuntimeSkillPath(
  candidate: string,
  skillName: string,
  ...
  for (let depth = 0; depth < 6; depth += 1) {
    if (await isLikelyIronworksRepoRoot(cursor)) return true;
...
async function pruneBrokenUnavailableIronworksSkillSymlinks(
  skillsHome: string,
  ...
  for (const entry of entries) {
    ...
    if (
      !(await isLikelyIronworksRuntimeSkillPath(resolvedLinkedPath, entry.name, {
    ...
    await onLog("stdout", `[ironworks] Removed stale Codex skill "${entry.name}" from ${skillsHome}\n`);
  }
}
...
export async function ensureCodexSkillsInjected(
  ...
  const allSkillsEntries = options.skillsEntries ?? (await readIronworksRuntimeSkillEntries({}, __moduleDir));
    ...
    try {
      ...
      if (existing?.isSymbolicLink()) {
        ...
        if (
          ...
          (await isLikelyIronworksRuntimeSkillPath(resolvedLinkedPath, entry.runtimeName))
        ) {
          ...
          await onLog("stdout", `[ironworks] Repaired Codex skill "${entry.runtimeName}" into ${skillsHome}\n`);
          continue;
        }
      }

      const result = await ensureIronworksSkillSymlink(entry.source, target, linkSkill);
      if (result === "skipped") continue;

      await onLog(
        "stdout",
        `[ironworks] ${result === "repaired" ? "Repaired" : "Injected"} Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[ironworks] Failed to inject Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }

  await pruneBrokenUnavailableIronworksSkillSymlinks(
    skillsHome,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const configuredCodexHome =
    typeof envConfig.CODEX_HOME === "string" && envConfig.CODEX_HOME.trim().length > 0
      ...
      : null;
  const codexSkillEntries = await readIronworksRuntimeSkillEntries(config, __moduleDir);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[ironworks] Codex session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[ironworks] Codex resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
