/**
 * G.8 — Kill-switch test for anthropic_api adapter.
 *
...
const DISABLE_ENV_KEY = "ADAPTER_DISABLE_ANTHROPIC_API";

describe("anthropic_api kill-switch (G.8)", () => {
  beforeEach(() => {
  ...
  it("testEnvironment returns fail when kill-switch is active", async () => {
    ...
    const result = await testEnvironment(
      { adapterType: "anthropic_api", config: { apiKey: "sk-ant-test" } },
      mockTransport as never,
    );
    expect(result.status).toBe("fail");
    expect(result.checks.some((c) => c.code === "anthropic_api_disabled")).toBe(true);
    expect(mockTransport.sendJson).not.toHaveBeenCalled();
  });

  it("testEnvironment proceeds normally when kill-switch is not set", async () => {
    // No ADAPTER_DISABLE_ANTHROPIC_API set — should attempt network probe (will fail, that is ok)
    const { testEnvironment } = await import("../test.js");
    const mockTransport = {
      sendJson: vi.fn().mockRejectedValue(new Error("network unreachable")),
      sendJsonStream: vi.fn(),
    };
    const result = await testEnvironment({ adapterType: "anthropic_api", config: {} }, mockTransport as never);
    // Should NOT have the kill-switch check code
    expect(result.checks.every((c) => c.code !== "anthropic_api_disabled")).toBe(true);
  });
  ...
  it("execute returns disabled error when kill-switch is active", async () => {
    ...
    const ctx = {
      runId: "run-1",
      agent: { id: "a1", companyId: "c1", name: "Test", adapterType: "anthropic_api", adapterConfig: {} },
    ...
    expect(result.errorCode).toBe("anthropic_api_disabled");
  ...
  it("execute proceeds normally when kill-switch is not set", async () => {
    ...
    const result = await execute(ctx as never, mockTransport as never);
    expect(result.errorCode).not.toBe("anthropic_api_disabled");
  });
});
