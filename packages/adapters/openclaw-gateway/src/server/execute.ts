...
import { asNumber, asString, buildIronworksEnv, parseObject } from "@ironworksai/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "ironworks";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "ironworks";
  if (input.strategy === "run") return `ironworks:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `ironworks:issue:${input.issueId}`;
  return fallback;
}
...
function resolveIronworksApiUrlOverride(value: unknown): string | null {
...
function buildIronworksEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const ironworksApiUrlOverride = resolveIronworksApiUrlOverride(ctx.config.ironworksApiUrl);
  const ironworksEnv: Record<string, string> = {
    ...buildIronworksEnv(ctx.agent),
    IRONWORKS_RUN_ID: ctx.runId,
  };

  if (ironworksApiUrlOverride) {
    ironworksEnv.IRONWORKS_API_URL = ironworksApiUrlOverride;
  }
  if (wakePayload.taskId) ironworksEnv.IRONWORKS_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) ironworksEnv.IRONWORKS_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) ironworksEnv.IRONWORKS_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) ironworksEnv.IRONWORKS_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) ironworksEnv.IRONWORKS_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    ironworksEnv.IRONWORKS_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return ironworksEnv;
}

function buildWakeText(payload: WakePayload, ironworksEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/ironworks-claimed-api-key.json";
  const orderedKeys = [
    "IRONWORKS_RUN_ID",
    "IRONWORKS_AGENT_ID",
    "IRONWORKS_COMPANY_ID",
    "IRONWORKS_API_URL",
    "IRONWORKS_TASK_ID",
    "IRONWORKS_WAKE_REASON",
    "IRONWORKS_WAKE_COMMENT_ID",
    "IRONWORKS_APPROVAL_ID",
    "IRONWORKS_APPROVAL_STATUS",
    "IRONWORKS_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = ironworksEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = ironworksEnv.IRONWORKS_API_URL ?? "<set IRONWORKS_API_URL>";

  const lines = [
    "Ironworks wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `IRONWORKS_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load IRONWORKS_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $IRONWORKS_API_KEY on every API call.",
    "- Use X-Ironworks-Run-Id: $IRONWORKS_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: IRONWORKS_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    '   - POST /api/issues/{issueId}/checkout with {"agentId":"$IRONWORKS_AGENT_ID","expectedStatuses":["todo","backlog","blocked"]}',
    ...
    "   - GET /api/companies/$IRONWORKS_COMPANY_ID/issues?assigneeAgentId=$IRONWORKS_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardIronworksPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  ironworksEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateIronworks = parseObject(payloadTemplate.ironworks);
  const workspace = asRecord(ctx.context.ironworksWorkspace);
  const workspaces = Array.isArray(ctx.context.ironworksWorkspaces)
    ? ctx.context.ironworksWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.ironworksRuntimeServiceIntents)
    ? ctx.context.ironworksRuntimeServiceIntents.filter((entry): entry is Record<string, unknown> =>
        Boolean(asRecord(entry)),
      )
    : [];

  const standardIronworks: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: ironworksEnv.IRONWORKS_API_URL ?? null,
  };

  if (workspace) {
    standardIronworks.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardIronworks.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardIronworks.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "ironworks-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const ironworksEnv = buildIronworksEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, ironworksEnv);
