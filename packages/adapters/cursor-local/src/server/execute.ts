...
export async function ensureCursorSkillsInjected(
  ...
  const skillsEntries =
    ...
    (options.skillsDir
      ...
      : await readIronworksRuntimeSkillEntries({}, __moduleDir));
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[ironworks] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog("stderr", `[ironworks] Removed maintainer-only Cursor skill "${skillName}" from ${skillsHome}\n`);
  }
  ...
  for (const entry of skillsEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[ironworks] ${result === "repaired" ? "Repaired" : "Injected"} Cursor skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[ironworks] Failed to inject Cursor skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const cursorSkillEntries = await readIronworksRuntimeSkillEntries(config, __moduleDir);
  const desiredCursorSkillNames = resolveIronworksDesiredSkillNames(config, cursorSkillEntries);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[ironworks] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: ironworksEnvNote.length,
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[ironworks] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
