/**
 * Skill management for openrouter_api adapter.
 *
 * OpenRouter's HTTP API is stateless — it cannot sync skill files to disk like CLI adapters.
...
/**
 * Return the skill snapshot for openrouter_api. Always reports mode "unsupported" because
      ...
      systemPromptSkills.length > 0
        ? [
            `Skills [${systemPromptSkills.join(", ")}] will be injected into the system prompt. ` +
              "OpenRouter API does not support native skill file sync — contents are inlined as text.",
          ]
        : [],
  };
}
