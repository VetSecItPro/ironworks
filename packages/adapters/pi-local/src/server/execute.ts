...
const IRONWORKS_SESSIONS_DIR = path.join(os.homedir(), ".pi", "ironworkss");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog("stderr", `[ironworks] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`);
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[ironworks] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[ironworks] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(IRONWORKS_SESSIONS_DIR, { recursive: true });
  return IRONWORKS_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(IRONWORKS_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Inject skills
  const piSkillEntries = await readIronworksRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveIronworksDesiredSkillNames(config, piSkillEntries);
  ...
  if (workspaceCwd) env.IRONWORKS_WORKSPACE_CWD = workspaceCwd;
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[ironworks] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const buildArgs = (sessionFile: string): string[] => {
    ...
    // Add Ironworks skills directory so Pi can load the ironworks skill
  ...
  if (canResumeSession && initialFailed && isPiUnknownSessionError(initial.proc.stdout, initial.rawStderr)) {
    await onLog(
      "stdout",
      `[ironworks] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
