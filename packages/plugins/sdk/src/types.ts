/**
 * Core types for the Ironworks plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@ironworksai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @ironworksai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Ironworks object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Ironworks UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Ironworks domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Ironworks domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Ironworks secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Ironworks secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@ironworksai/plugin-sdk/ui`
 */
