...
export const databaseConfigSchema = z.object({
  ...
  embeddedPostgresDataDir: z.string().default("~/.ironworks/instances/default/db"),
  embeddedPostgresPort: z.number().int().min(1).max(65535).default(54329),
  backup: databaseBackupConfigSchema.default({
    enabled: true,
    intervalMinutes: 60,
    retentionDays: 30,
    dir: "~/.ironworks/instances/default/data/backups",
  }),
});

export const loggingConfigSchema = z.object({
  mode: z.enum(["file", "cloud"]),
  logDir: z.string().default("~/.ironworks/instances/default/logs"),
});
...
export const storageLocalDiskConfigSchema = z.object({
  baseDir: z.string().default("~/.ironworks/instances/default/data/storage"),
});

export const storageS3ConfigSchema = z.object({
  bucket: z.string().min(1).default("ironworks"),
...
export const storageConfigSchema = z.object({
  ...
  localDisk: storageLocalDiskConfigSchema.default({
    baseDir: "~/.ironworks/instances/default/data/storage",
  }),
...
export const secretsLocalEncryptedConfigSchema = z.object({
  keyFilePath: z.string().default("~/.ironworks/instances/default/secrets/master.key"),
});
...
export const secretsConfigSchema = z.object({
  ...
  localEncrypted: secretsLocalEncryptedConfigSchema.default({
    keyFilePath: "~/.ironworks/instances/default/secrets/master.key",
  }),
});

export const ironworksConfigSchema = z
  .object({
...
export type IronworksConfig = z.infer<typeof ironworksConfigSchema>;
