...
describe('createObserver (http sink)', () => {
  ...
  it('does not throw if the collector is unreachable', async () => {
    const fetchSpy: FetchMock = vi.fn(async () => { throw new Error('network down'); });
    const observer = createObserver({
      sink: 'http:https://telemetry.example.com/collect',
      fetchImpl: fetchSpy,
    });
  ...
  it('drops events + warns when inflight Set exceeds cap', async () => {
    ...
    const warningFired = stderrWrite.mock.calls.some(call =>
      (call[0] as string).includes('telemetry queue full') ||
      (call[0] as string).includes('dropped')
    );
...
describe('env-based observer (process.env.IRONWORKS_TELEMETRY_SINK)', () => {
  ...
  it('invalid sink URL falls back to stdout with stderr warning', () => {
    ...
    const stdoutWrite = vi.spyOn(process.stdout, 'write').mockImplementation(() => true);
    emitCallEvent({
      timestamp: '2026-04-19T12:00:00Z', request_id: 'r', workspace_id: 'w',
      adapter_type: 'poe_api', model: 'x', duration_ms: 0, status: 'success',
    });
