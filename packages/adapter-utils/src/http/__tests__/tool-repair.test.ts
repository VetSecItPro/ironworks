...
import { attemptToolRepair, type RepairPrompter, type RepairOutcome } from '../tool-repair.js';
...
describe('attemptToolRepair — successful repair', () => {
  ...
  it('passes validation errors to the prompter', async () => {
    const bad: ToolInvocation = {
      toolCallId: 'c1', toolName: 'get_weather', args: { units: 'kelvin' },
    };
    const prompter = vi.fn<RepairPrompter>(async () => ({
      toolCallId: 'c1', toolName: 'get_weather', args: { city: 'Paris' },
    }));
...
describe('attemptToolRepair — repair fails', () => {
  it('returns failed outcome when repaired args still invalid', async () => {
    const bad: ToolInvocation = {
      toolCallId: 'c1', toolName: 'get_weather', args: {},
    };
    const prompter: RepairPrompter = vi.fn(async () => ({
      toolCallId: 'c1', toolName: 'get_weather', args: { city: 123 as unknown as string },  // still bad
    }));
