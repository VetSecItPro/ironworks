...
const IRONWORKS_SKILL_ROOT_RELATIVE_CANDIDATES = ["../../skills", "../../../../../skills"];
...
export function buildIronworksEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(process.env.IRONWORKS_LISTEN_HOST ?? process.env.HOST ?? "localhost");
...
export async function readIronworksSkillMarkdown(moduleDir: string, skillKey: string): Promise<string | null> {
